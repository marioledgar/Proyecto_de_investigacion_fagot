...
Para llegar a la ecuación \ref{ecuacion_mas} tenemos que primero encontrar la ecuación del punto $x = 0$.
...
\begin{equation}\label{suma_fourier}
...
\begin{subproof}
    ...
    \begin{align}
        ...
        &= \half \left[ \int_0^{2\pi} \cos ((n - m) x) \ddint x + \int_0^{2\pi} \cos((n + m) x) \ddint x \right] \label{integral_fea}
    \end{align}
    Por un momento, supongamos que $n \not= m$.

    Es fácil ver que para cualquier $n \not= 0$ se cumple que
    \[\int_0^{2\pi} \sen (nx) \ddint x = \int_0^{2\pi} \cos (nx) \ddint x = 0.\]
    Esto se puede ver visualmente en la figura \ref{integral_seno}: las áreas naranjas y las verdes se cancelan.
    Lo mismo pasa con el coseno, solo que con cierto desfase.
    \begin{figure}[ht]
        \centering
        \includegraphics[width=0.5\linewidth]{partes/apendices/figuras/integral_seno.png}
...
Ahora, fijamos $k \geq 0$ y multiplicamos ambos lados de la ecuación \ref{suma_fourier} por $\cos(kx)$ e integramos:
\begin{equation*}

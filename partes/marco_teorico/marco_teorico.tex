...
\begin{figure}
    \centering
    \subcaptionbox{}
    {\includegraphics[height=5cm]{partes/marco_teorico/figuras/muelles/Muelle_abajo.png}}
    \subcaptionbox{}
    {\includegraphics[height=5cm]{partes/marco_teorico/figuras/muelles/Muelle_equilibrio.png}}
    \subcaptionbox{}
    {\includegraphics[height=5cm]{partes/marco_teorico/figuras/muelles/Muelle_arriba.png}}
    \caption{MAS en un muelle. Fuente: Wikipedia.}\label{muelle}
\end{figure}
...
Igualando la ley de Hooke (\ref{hooke}) a la segunda ley de Newton (\ref{newton2}) nos queda que

\begin{equation}\label{newhoo}
    ma = -sx,
\end{equation}
...
\begin{equation}\label{ecuacion_dif}
    \ddot{x} + \omega^2x = 0.
\end{equation}
...
\begin{align}
    ...
    & = A \sen (\omega t + \varphi_0). \label{MAS}
\end{align}
...
Al ver esto, nos damos cuenta de que la ecuación de la aceleración es idéntica a la ecuación \ref{ecuacion_dif} salvo por el cambio de variable que hicimos.
...
Esto se puede ver en la figura \ref{sumaMAS}.

En el caso de las ondas de sonido, si las frecuencias son similares, esto produce un sonido que percibimos como si fuera pulsante pulsante.
...
\begin{figure}[t!]
    \centering
    \subcaptionbox{}{\includegraphics[height=3cm]{partes/marco_teorico/figuras/pulsos/Onda1.png}}
    \subcaptionbox{}{\includegraphics[height=3cm]
    {partes/marco_teorico/figuras/pulsos/Onda 2.png}}
    \subcaptionbox{}{\includegraphics[height=3cm]{partes/marco_teorico/figuras/pulsos/Pulsos.png}}
...
En la figura \ref{ondasdim} se pueden ver ejemplos de los tres tipos de ondas.
...
Además, si el foco de la onda es el origen y la onda se mueve en un medio sin obstáculos en el que la velocidad de propagación es la misma en todas las direcciones,
los  frentes de onda son circunferencias centradas en el origen en el caso de las ondas bidimensionales y esferas centradas en el origen en el caso de las tridimensionales.
...
\begin{figure}[ht!]
    ...
    \subcaptionbox{Onda unidimensional \dots}{\includegraphics[height=3.1cm]{
        partes/marco_teorico/figuras/ondas_dimensiones/Unidimensional.png}}
    \subcaptionbox{\dots bidimensional \dots}{\includegraphics[height=3.1cm]{partes/marco_teorico/figuras/ondas_dimensiones/Bidimensional.png}}
    \subcaptionbox{\dots y tridimensional}{\includegraphics[height=3.3cm]{partes/marco_teorico/figuras/ondas_dimensiones/Tridimensional.png}}
...
\begin{figure}[ht!]
    \centering
    \subcaptionbox{Una onda longitudinal}{\includegraphics[height=5cm]{partes/marco_teorico/figuras/ondas_direccion/Longitudinal.png}}\hspace{2cm}
    \subcaptionbox{\dots y una transversal}{\includegraphics[height=5cm]{partes/marco_teorico/figuras/ondas_direccion/Transversal.png}}
...
Esto se ve mejor con un dibujo: en la figura \ref{ondasmag} se representan la amplitud $A$, el periodo $T$ y la longitud de onda $\lambda$. La primera imagen muestra como cambia la posición de una sola partícula según avanza el tiempo y la segunda, el estado global del medio en instante de tiempo.

\begin{figure}[ht!]
    \centering
    \subcaptionbox{Desplazamiento respecto al tiempo en una partícula fija.}{\includegraphics[height=3cm]{partes/marco_teorico/figuras/magnitudes_ondas/onda_amplitud_periodo.png}}\hspace{2cm}
    \subcaptionbox{Desplazamiento respecto a la posición en un instante fijo.}{\includegraphics[height=3cm]{partes/marco_teorico/figuras/magnitudes_ondas/onda_longitud_frecuencia.png}}
...
Derivamos esta ecuación en el Apéndice \ref{demostraciones}.
...
Por ejemplo, en la figura \ref{sumaMAS}, la curva verde es la envolvente de la azul.
...
En el apéndice \ref{demostraciones} derivamos la siguiente fórmula:
\begin{equation*}
    I = \text{cte.} \cdot A^2.
\end{equation*}
Esto y la proporcionalidad entre distancia y intensidad nos da que la amplitud de un frente de onda a una distancia $r$ del foco viene dada por
\begin{equation*}
\begin{aligned}
    A &= P,\\[2pt]
...
Sin embargo, si analizamos la caída de la intensidad de una onda real, vemos que es más rápida que la calculada por la atenuación, como se ve en la figura \ref{absorcion}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=5.5cm]{partes/marco_teorico//figuras/absorcion_vs_atenuacion.png}
...
Las siguiente propiedad es una de las más importantes, y es útil para demostrar el comportamiento de otras.
...
Esto se ve mejor con una imagen: la figura \ref{huygens} muestra esto perfectamente para ondas planas y esféricas.
\begin{figure}
    \centering
    \includegraphics[width=0.4\linewidth]{partes/marco_teorico//figuras/huygens.png}
...
Esto se ve en la figura \ref{reflexion_refraccion}.
...
donde se sigue la nomenclatura de la figura \ref{reflexion_refraccion} para los ángulos y $v_i$ y $v_t$ son, respectivamente, la velocidad en el primer y segundo medio.

En la figura \ref{refraccion_lapiz} se ve el efecto de la luz sobre las imágenes.

\begin{figure}
    \centering
    \includegraphics[width=0.3\linewidth]{partes/marco_teorico//figuras/reflexion_refraccion.png}
    \caption{Reflexión y refracción de una onda. Fuente: Wikipedia.}
    \label{reflexion_refraccion}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.25\linewidth]{partes/marco_teorico//figuras/refraccion_real.png}
    \caption{La refracción hace que parezca que el lápiz está doblado. Fuente: Wikipedia.}
    \label{refraccion_lapiz}
\end{figure}

Como se ve en la figura \ref{reflexion_refraccion}, la reflexión puede ocurrir la reflexión a la vez que la refracción.
...
Este es el motivo del por qué el sonido de la sirena de una ambulancia se hace más agudo al acercarse a nosotros y más grave al  alejarse.
...
Volviendo al tema, no todos estaban de acuerdo en las condiciones que debía cumplir un función para poder descomponerse en serie trigonométrica.
...
\begin{equation}\label{difcuerda}
...
Imponiendo las condiciones de contorno $u(x, 0) = f(x)$ e $u(0, t) = u(1, t) = 0$, llegó a que
...
Por ejemplo, si $f(x) = x^2\sen x$ para $0 \leq x \leq \pi$, la cuerda vibraría como se ve en la figura \ref{vibcue} (en este caso $L = \pi$).

\begin{figure}
    \centering
    \includegraphics[height=5cm]{partes/marco_teorico/figuras/vibracion_cuerda.png}
    \caption{Vibración de una cuerda con forma $f(x) = x^2 \sen x$. Elaboración propia.}
    \label{vibcue}
\end{figure}

La ecuación \ref{cuerda} tiene sentido para cualquier función $f$; sin embargo, la ecuación \ref{difcuerda} solo tiene sentido para funciones con segunda derivada.
...
Por ejemplo, si hacemos que la cuerda tenga un pico, como en la figura \ref{trian}, en principio no se podría aplicar nuestra fórmula, ya que la función no tiene segunda derivada en este pico.
\begin{figure}[ht!]
    \centering
    \subcaptionbox{Función original.}{\includegraphics[height=3cm]{partes/marco_teorico/figuras/onda_triangular/original.png}}\hspace{1cm}
    \subcaptionbox{Aproximación para $n=3$}{\includegraphics[height=3cm]{partes/marco_teorico/figuras/onda_triangular/fourier_aproximada.png}}
...
hasta que Daniel Bernoulli (1604-1705), hijo de Johann Bernoulli, dio el saltó crucial: consideró representar $f$ como la suma de varios (infinitos) senos y cosenos \cite{bernoulli}, lo que actualmente se conoce como la serie de Fourier de $f$:
...
La demostración de la convergencia de la serie de Fourier se puede encontrar en el apéndice \ref{demostraciones}.
...
\end{equation*}
Se pueden encontrar más detalles en el apéndice \ref{demostraciones}.
...
\begin{eq}\label{valores_enteros}
...
La ecuación \ref{valores_enteros} se deduce fácilmente de la definición de los coeficientes y la transformada:
\begin{align*}
...
En primer lugar, tomamos $N$ muestras de nuestra función separadas por un tiempo $T$ entre ellas. Denotamos estas muestras por $f[0]$, $f[T]$, \dots, $f[(N - 1)T]$. Esto se ve en la figura \ref{sampling} (a).

\begin{figure}[ht!]
    \centering
    \subcaptionbox{Muestreo.}{\includegraphics[width = 7cm]{partes/marco_teorico/figuras/DFT/sampling.png}}
    \subcaptionbox{Periodificación.}{\includegraphics[width = 7cm]{partes/marco_teorico/figuras/DFT/periodica.png}}
...
Como se ve en la figura \ref{sampling} (b), ahora estamos suponiendo que nuestra función es periódica, y que la DFT funcionaría para toda la onda; aunque no es así y tendremos que calcularla para cada intervalo de tiempo.
...
En física, el sonido es una onda mecánica, una vibración,
que se propaga a través de un medio, ya sea líquido sólido o gas.
En psicología y fisiología, el sonido es la recepción de esta vibración
por el sentido del oído, y su procesamiento por parte del cerebro.
...
El fagot es un instrumento de viento madera, de taladro cónico, 
